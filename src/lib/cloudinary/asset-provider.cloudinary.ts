...
export class CloudinaryAssetProvider implements AssetStorageProvider {
    ...
    * Tries `image` first (the most common type for tour assets) and only
    * falls back to video/raw in parallel when image returns a 404.
    */
    async getAssetByChecksum(checksum: string): Promise<CloudinaryApiResource> {
        // Fast-path: image (covers virtually all tour uploads)
        try {
