...
import { ASSET_TYPE, AssetType, VISIBILITY } from "@/constants/common/asset.const";
...
export async function resolveDocuments(
    ...
    // We separate "already resolved" items from "needs upload" items so we can
    // fire all new uploads in a single parallel batch rather than one-by-one.
    ...
    if (pending.length > 0) {
        try {
            uploadedIds = await uploadAssets(
                ...
                session
                // concurrency defaults to 4 inside uploadAssets
            );
        } catch (err) {
                ...
                for (const p of pending) {
                    try {
                        ...
                        if (innerErr instanceof MongoServerError && innerErr.code === 11000 && innerErr.keyPattern?.checksum) {
                            ...
                            // Fix: create Asset document correctly
                            const assetDoc = await AssetModel.create([{
                                file: reused._id,
                                assetType: assetType ?? ASSET_TYPE.DOCUMENT,
                                title: p.type || "Uploaded Asset",
                                visibility: VISIBILITY.PUBLIC
                            }], { session });

                            uploadedIds.push(assetDoc[0]._id as Types.ObjectId);
                        } else {
                            throw innerErr;
                        }
                    }
    ...
    /** 4️⃣ Identify assets that are no longer referenced */
    const assetsToDelete = existingDocs
        .map(d => d.asset)
        .filter(id => !usedAssetIds.has(id.toString()));

    return { resolvedDocs: finalDocuments, assetsToDelete };
}
