...
export const Step0BasicInfoSchema = Yup.object().shape({
    ...
    seo: Yup.object()
        ...
        .optional(),
    tags: Yup.array().of(Yup.string().max(20, 'Tag must be less than 20 characters')).optional(),
});

export const Step1BangladeshSchema = Yup.object().shape({
    tourType: Yup.string().oneOf(Object.values(TRAVEL_TYPE)).required('Tour type is required'),
    division: Yup.string().required('Division is required'),
    district: Yup.string().required('District is required'),
    accommodationType: Yup.array().of(
        Yup.string().oneOf(Object.values(ACCOMMODATION_TYPE))
    ).optional(),
...
export const Step2ContentSchema = Yup.object().shape({
    ...
    itinerary: Yup.array().of(
        Yup.object().shape({
            day: Yup.number().required('Day number is required').min(1, 'Day must be at least 1'),
            title: Yup.string().optional(),
            description: Yup.string().optional(),
            mealsProvided: Yup.array().of(
                Yup.string().oneOf(Object.values(MEALS_PROVIDED))
            ).optional(),
            accommodation: Yup.string().optional(),
            activities: Yup.array().of(Yup.string()).optional(),
            travelDistance: Yup.string().optional(),
            travelMode: Yup.string().oneOf(Object.values(TRANSPORT_MODE)).optional(),
...
export const Step3LogisticsSchema = Yup.object().shape({
    ...
    transportModes: Yup.array().of(
        Yup.string().oneOf(Object.values(TRANSPORT_MODE))
    ).optional(),

    pickupOptions: Yup.array().of(
        Yup.object().shape({
...
export const Step4PricingSchema = Yup.object().shape({
    ...
    discounts: Yup.array().of(
        Yup.object().shape({
            ...
            code: Yup.string().when('type', {
                ...
                then: () => Yup.string().required('Promo code is required for promo discounts'),
                otherwise: () => Yup.string().optional(),
            }),
            validFrom: Yup.date()
                .typeError('Valid from must be a valid date')
                .optional()
                .test('not-past-if-set', 'Valid from cannot be in the past if set', function (value) {
            ...
            validUntil: Yup.date()
                ...
                .optional()
                .test('is-after-valid-from', 'Valid until must be after valid from', function (value) {
                    const { validFrom } = this.parent;
                    if (!validFrom || !value) return true;
                    return new Date(value) > new Date(validFrom);
                })
                .test('not-past-if-set', 'Valid until cannot be in the past', function (value) {
                    // Allow empty/undefined values
                    if (!value) return true;
                    return new Date(value) >= new Date(new Date().setHours(0, 0, 0, 0));
                }),
    ...
    duration: Yup.object()
        .shape({
            days: Yup.number().required('Days is required').min(1, 'At least 1 day is required'),
            nights: Yup.number().min(0, 'Nights cannot be negative').optional(),
        })
        .optional(),

    operatingWindows: Yup.array().of(
        Yup.object().shape({
            startDate: Yup.date()
            ...
            endDate: Yup.date()
                ...
                .test('min-duration', 'End date must allow for at least one day of operation', function (value) {
                    const { startDate } = this.parent;
                    if (!startDate || !value) return true;
                    const start = new Date(startDate);
                    const end = new Date(value);
        ...
        .test('non-overlapping-windows', 'Operating windows cannot overlap', function (windows) {
    ...
    departures: Yup.array().of(
        Yup.object().shape({
            date: Yup.date()
                .typeError('Departure date must be a valid date')
                .required('Departure date is required')
                .test('not-past', 'Departure date cannot be in the past', function (value) {
                    return new Date(value) >= new Date(new Date().setHours(0, 0, 0, 0));
                })
                .test('within-operating-window', 'Departure date must be within an operating window', function (value) {
                ...
                .test('has-enough-duration', 'Departure must allow for tour duration', function (value) {
                    const { duration, operatingWindows } = this.parent?.parent || {};

                    if (!duration?.days || !operatingWindows || operatingWindows.length === 0) return true;

                    const departureDate = new Date(value);
                    const tourDurationDays = duration.days;

                    // Find the operating window for this departure
                    const operatingWindow = operatingWindows.find((window: OperatingWindowDTO) => {
                        const windowStart = new Date(window.startDate);
                        const windowEnd = new Date(window.endDate);
                        return departureDate >= windowStart && departureDate <= windowEnd;
                    });
        ...
        .optional()
        .test('unique-departure-dates', 'Departure dates must be unique', function (departures) {
        ...
        .test('departures-within-windows', 'All departures must be within operating windows', function (departures) {
            const operatingWindows = this.parent?.operatingWindows;

            // If no departures or no operating windows, validation passes
            if (!departures || departures.length === 0 || !operatingWindows || operatingWindows.length === 0) {
                return true;
            }
            ...
            for (const departure of departures) {
                ...
                const isWithinWindow = operatingWindows.some((window: OperatingWindowDTO) => {
                    const windowStart = new Date(window.startDate);
                    const windowEnd = new Date(window.endDate);
                    return departureDate >= windowStart && departureDate <= windowEnd;
                });
    ...
    cancellationPolicy: Yup.object()
        .shape({
            ...
            rules: Yup.array()
                .of(
                    Yup.object().shape({
                        daysBefore: Yup.number()
                            ...
                            .test('unique-days-before', 'Days before values must be unique', function (value, context) {
                                ...
                                const duplicate = rules.some((rule, index) =>
                                    rule.daysBefore === value && index !== parseInt(currentIndex)
                                );
                                return !duplicate;
                            }),
                        refundPercent: Yup.number()
                            ...
                            .test('descending-refund', 'Refund percentage should generally decrease as days before increases', function (value, context) {
                                const rules = context.parent;
                                if (!rules || !Array.isArray(rules) || rules.length <= 1) return true;
                                ...
                                const sortedIndex = sortedRules.findIndex(rule =>
                                ...
                                if (sortedIndex > 0) {
                                    ...
                                    if (currentRule.refundPercent > prevRule.refundPercent) {
                                        return this.createError({
                                            message: 'Refund percentage should not increase as cancellation date gets closer'
                                        });
                                    }
                                }
                ...
                .test('covers-all-periods', 'Cancellation rules should cover from 0 days to a reasonable maximum', function (rules) {
                    if (!rules || rules.length === 0) return true;

                    // Sort by daysBefore
                    const sortedRules = [...rules].sort((a, b) => a.daysBefore - b.daysBefore);

                    // Check if first rule covers 0 days
                    if (sortedRules[0].daysBefore > 0) {
                        return this.createError({
                            message: 'First cancellation rule should cover 0 days (day of departure)'
                        });
                    }

                    // Check if last rule has reasonable maximum (e.g., 30, 60, 90 days)
                    const lastRuleDays = sortedRules[sortedRules.length - 1].daysBefore;
                    if (lastRuleDays < 7) {
                        return this.createError({
                            message: 'Last cancellation rule should cover at least 7 days before departure'
                        });
                    }
