...
function sanitizeAddress(
  address: ITraveler["address"],
): IBookingPopulated["traveler"]["address"] {
...
async function resolveAssetUrls(
  ...
  const assetFiles = await AssetFileModel.find<AssetFileDoc>({
    _id: { $in: fileIds },
  })
    .select("publicUrl")
    .lean();
...
async function getBookings(
  req: NextRequest,
): Promise<HandlerResult<{ data: IBookingPopulated[]; meta: PaginationMeta }>> {
  ...
  const status: BookingStatus[] | undefined = statusParam
    ? statusParam
        ...
        .filter((s): s is BookingStatus =>
          (Object.values(BOOKING_STATUS) as string[]).includes(s),
        )
    : undefined;
  ...
  const paymentStatus: BookingPaymentStatus[] | undefined = paymentStatusParam
    ? paymentStatusParam
        .split(",")
        .filter((s): s is BookingPaymentStatus =>
          (Object.values(BOOKING_PAYMENT_STATUS) as string[]).includes(s),
        )
    : undefined;
  ...
  pipeline.push({
    $unwind: { path: "$travelerObj", preserveNullAndEmptyArrays: false },
  });
  ...
  pipeline.push({
    $unwind: { path: "$userObj", preserveNullAndEmptyArrays: false },
  });
  ...
  pipeline.push({
    $unwind: { path: "$tourObj", preserveNullAndEmptyArrays: false },
  });
  ...
  if (tourTitle) {
    pipeline.push({
      $match: { "tourObj.title": { $regex: tourTitle, $options: "i" } },
    });
  }
  ...
  function toBookingPopulated(
    item: RawBookingAggregationOutput,
  ): IBookingPopulated {
    return {
