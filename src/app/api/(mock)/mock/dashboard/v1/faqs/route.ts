...
const schema = z.object({
    faqsDateRangeFrom: z.string().transform((str) => new Date(str)),
    faqsDateRangeTo: z.string().transform((str) => new Date(str)),
});
...
export async function GET(request: NextRequest) {
    try {
        ...
        const { faqsDateRangeFrom: from, faqsDateRangeTo: to } = parsed.data;
        const data = getMasterMockData()
            .faqs.filter((f) => isInRange(f.createdAt, from, to))
            .slice(0, 100);
