...
export default function ReviewsToolbar({
    ...
    const activeFiltersCount = useMemo(() => {
        ...
        if (toolbar.filters.userId) count++;
        if (toolbar.filters.tripType) count++;
        if (toolbar.filters.isVerified) count++;
            ...
            <div className="p-4 space-y-4">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="flex items-center gap-2">
                        <motion.label
                            whileHover={{ scale: 1.02 }}
                            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border cursor-pointer transition-all ${toolbar.filters.isVerified
                        ...
                        >
                            <input
                                type="checkbox"
                                checked={toolbar.filters.isVerified ?? false}
                                onChange={(e) => setFilter("isVerified", e.target.checked)}
                                className="w-4 h-4 text-emerald-600 bg-white border-slate-300 rounded focus:ring-2 focus:ring-emerald-500/20"
                    ...
                    <div className="relative ml-auto">
                        <motion.button
                            whileHover={{ scale: 1.02 }}
                            whileTap={{ scale: 0.98 }}
                            data-testid="toolbar-bulk-menu"
                            type="button"
                            disabled={toolbar.selectedIds.length === 0}
                            className="inline-flex items-center gap-2 px-4 py-1.5 text-sm font-medium text-slate-700 bg-white rounded-lg border border-slate-200 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-300/50 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                            onClick={() => setShowBulkMenu(!showBulkMenu)}
                            aria-label="Bulk actions menu"
                        >
                            <FaDownload className="w-3.5 h-3.5" aria-hidden />
                            Bulk Actions
                            {toolbar.selectedIds.length > 0 && (
                                <span className="px-1.5 py-0.5 text-xs font-bold bg-slate-700 text-white rounded-full">
                                    {toolbar.selectedIds.length}
                                </span>
                            )}
                            <FaChevronDown className="w-3 h-3 ml-1" />
                        </motion.button>
                        ...
                        <AnimatePresence>
                            {showBulkMenu && toolbar.selectedIds.length > 0 && (
                                <motion.div
                                ...
                                >
                                    <button
                                        onClick={() => {
                                            onBulkAction(toolbar.selectedIds, "export_csv");
                                    ...
                                    <button
                                        onClick={() => {
                                            onBulkAction(toolbar.selectedIds, "approve");
                                            setShowBulkMenu(false);
                                        }}
                                    ...
                                    <button
                                        onClick={() => {
                                            onBulkAction(toolbar.selectedIds, "delete");
                                            setShowBulkMenu(false);
                                        }}
                    ...
                    >
                        <div id="advanced-filters" className="p-4 bg-slate-50/50">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                ...
                                {/* User ID */}
                                <div className="space-y-2">
                                    <label htmlFor="userId" className="flex items-center gap-2 text-sm font-medium text-slate-700">
