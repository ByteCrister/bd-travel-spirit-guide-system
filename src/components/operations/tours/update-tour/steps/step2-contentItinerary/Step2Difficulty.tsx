...
const NEU_INPUT = 'rounded-xl bg-[#E7E5E4] text-[#1E2938] placeholder:text-[#1E2938]/40 font-[family-name:var(--font-jetbrains-mono)] text-sm  border-none focus:outline-none focus:ring-2 focus:ring-[#006666]/50 transition-all duration-200 px-3 h-11 w-full';
            ...
            <div className="px-6 py-5 space-y-5">
                ...
                <div>
                    <Select value={values.difficulty || ''} onValueChange={handleDifficultyChange}>
                        ...
                        <SelectContent className="bg-[#E7E5E4] border border-white/60  rounded-xl">
                            {difficultyLevels.map((level) => {
                                const cfg = DIFFICULTY_CONFIG[level];
                                return (
                                    <SelectItem key={level} value={level} className="focus:bg-[#006666]/10 rounded-lg">
                                        <div className="flex items-center gap-3 py-1">
                                            <div className={`w-8 h-8 rounded-xl ${cfg?.accentBg} flex items-center justify-center text-white `}>
                                                {cfg?.icon}
                                            </div>
                                            <div>
                ...
                {values.difficulty && currentCfg && (
                    <div className={`${NEU_CARD_SM} p-5 space-y-4`}>
                        {/* Selected badge row */}
                        <div className="flex items-center gap-3">
                            <div className={`h-10 w-10 rounded-xl ${currentCfg.accentBg} flex items-center justify-center text-white `}>
                                {currentCfg.icon}
                            </div>
                            <div>
                                ...
                                return (
                                    <div key={level} className="flex items-center">
                                        <div className={[
                                            ...
                                            filled ? cfg?.dotColor : 'bg-[#E7E5E4] ',
                                        ].join(' ')} />
                                        {i < difficultyLevels.length - 1 && (
                                            <div className={`h-1 w-8 transition-all duration-300 ${filled ? cfg?.dotColor + ' opacity-40' : 'bg-[#E7E5E4] '}`} />
                                        )}
                                    </div>
                                );
                            })}
