...
export default function ProfileHeader({ baseUser, fullUser }: ProfileHeaderProps) {
  ...
  const getDisplayName = () => {
    if (baseUser.role === USER_ROLE.GUIDE && isGuideUser(fullUser)) {
      return fullUser.owner.name;
    }

    if (isEmployeeUser(fullUser) && fullUser.fullName) {
  ...
  const getAvatar = () => {
    if (baseUser.role === USER_ROLE.GUIDE && isGuideUser(fullUser)) {
      return fullUser.owner.avatar;
    }

    if (isEmployeeUser(fullUser)) {
