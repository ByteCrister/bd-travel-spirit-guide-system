...
} from "@/types/dashboard/dashboard-store.type";
    ...
    (set, get) => ({
      ...
      fetchStats: async (forceRefresh = false) => {
        ...
        try {
          ...
          const response = await api.get<ApiResponse<DashboardStatsBundle>>(
            API_PATHS.stats,
            { params },
          );
      ...
      fetchProfile: async (forceRefresh = false) => {
        ...
        try {
          const response = await api.get<
            ApiResponse<{ companyInfo: CompanyInfo; ownerInfo: OwnerInfo }>
          >(API_PATHS.profile);
          if (response.data.error) throw new Error(response.data.error);
      ...
      fetchTours: async (forceRefresh = false) => {
        ...
        try {
          ...
          const response = await api.get<ApiResponse<TourSummary[]>>(
            API_PATHS.tours,
            { params },
          );
          if (response.data.error) throw new Error(response.data.error);
          ...
          if (!toursData || !Array.isArray(toursData))
            throw new Error("Invalid tours data");
          set({
            toursData,
            isLoadingTours: false,
            isInitialData: response.data.isInitialData ?? false,
      ...
      fetchBookings: async (forceRefresh = false) => {
        ...
        try {
          ...
          const response = await api.get<ApiResponse<BookingSummary[]>>(
            API_PATHS.bookings,
            { params },
          );
          if (response.data.error) throw new Error(response.data.error);
      ...
      fetchEmployees: async (forceRefresh = false) => {
        ...
        try {
          ...
          const response = await api.get<ApiResponse<EmployeeSummary[]>>(
            API_PATHS.employees,
            { params },
          );
          if (response.data.error) throw new Error(response.data.error);
      ...
      fetchRunningTours: async (forceRefresh = false) => {
        ...
        try {
          ...
          const response = await api.get<ApiResponse<RunningTourInfo[]>>(
            API_PATHS.runningTours,
            { params },
          );
          if (response.data.error) throw new Error(response.data.error);
          const runningToursData = response.data.data;
          if (!runningToursData || !Array.isArray(runningToursData))
            throw new Error("Invalid running tours");
          set({
            runningToursData,
            isLoadingRunningTours: false,
            isInitialData: response.data.isInitialData ?? false,
      ...
      fetchFaqs: async (forceRefresh = false) => {
        ...
        try {
          ...
          const response = await api.get<ApiResponse<FAQSummary[]>>(
            API_PATHS.faqs,
            { params },
          );
          if (response.data.error) throw new Error(response.data.error);
      ...
      fetchRefunds: async (forceRefresh = false) => {
        ...
        try {
          ...
          const response = await api.get<ApiResponse<RefundSummary[]>>(
            API_PATHS.refunds,
            { params },
          );
          if (response.data.error) throw new Error(response.data.error);
          ...
          if (!refundsData || !Array.isArray(refundsData))
            throw new Error("Invalid refunds data");
          set({
            refundsData,
            isLoadingRefunds: false,
            isInitialData: response.data.isInitialData ?? false,
      ...
      setSectionDateRange: (
        ...
        set((state) => {
          ...
          if (section === "runningToursDateRange")
            delete nextTs.runningToursData;
          if (section === "faqsDateRange") delete nextTs.faqsData;
          if (section === "refundsDateRange") delete nextTs.refundsData;
          if (section === "transactionsDateRange")
            delete nextTs.transactionsFirst;
          return {
        ...
        else if (section === "runningToursDateRange")
          void g.fetchRunningTours(true);
        else if (section === "faqsDateRange") void g.fetchFaqs(true);
        else if (section === "refundsDateRange") void g.fetchRefunds(true);
        else if (section === "transactionsDateRange")
          void g.fetchTransactionsFirstPage(true);
      },
      ...
      exportToCSV: async (type) => {
        ...
        try {
          ...
          // Assume the API returns JSON conforming to ApiResponse<{ csvContent: string }>
          const response = await api.get<ApiResponse<{ csvContent: string }>>(
            API_PATHS.export,
            {
              params,
              // Remove responseType: 'blob' – we expect JSON now
            },
          );
          ...
          const binaryString = atob(data.csvContent);
          ...
          link.setAttribute(
            "download",
            `dashboard_${type}_${format(new Date(), "yyyy-MM-dd")}.csv`,
          );
