...
interface TourFaqFilterParams extends PaginationParams {
  search?: string;
}

interface TourReviewFilterParams extends PaginationParams {
  search?: string;
  rating?: 1 | 2 | 3 | 4 | 5; // Changed from just 1-4 to include 5
...
interface ListCache<T> {
  ...
  params: {
    ...
    filters?: TourFilterOptions | TourReviewFilterParams | TourFaqFilterParams;
  };
  meta?: {
...
interface ToutDetailState {
  ...
  fetchReviews: (
    tourId: string,
    params?: Partial<TourReviewFilterParams>,
    force?: boolean
  ...
  fetchFaqs: (
    tourId: string,
    params?: Partial<TourFaqFilterParams>,
    force?: boolean
...
// Store
// --------------------
export const useTourDetailStore = create<ToutDetailState>()(
  devtools(
    persist(
      (set, get) => ({
        company: undefined,
        ...
        fetchTours: async (
          ...
          // Update BOTH params and tourFilters immediately
          set((s) => ({
            params: {
              ...s.params,
              tours: {
          ...
          // Now use the filters from the updated state (or the passed filters)
          const currentFilters = filters !== undefined ? filters : state.tourFilters;
        ...
        fetchReviews: async (tourId, overrideParams = {}, force = false) => {
          ...
          if (!state.params.tourReviews[tourId])
            state.params.tourReviews[tourId] = {
              ...defaultPagination,
              search: '',
              rating: undefined,
              approvedOnly: false
          ...
          // Merge with overrideParams
          const params: TourReviewFilterParams = {
            ...defaultPagination,
            search: '',
            approvedOnly: false,
            ...currentParams,
          ...
          // Update the cache key to include filter params
          ...
          // Build request parameters including filters
          const requestParams: Record<string, unknown> = {
            page: params.page,
            limit: params.limit,
          };
          ...
          if (params.rating) requestParams.rating = params.rating;
          if (params.approvedOnly) requestParams.approvedOnly = params.approvedOnly;

          const url = `${URL_AFTER_API}/${tourId}/tour-insights/reviews`;
          const reqKey = makeRequestKey("GET", url, requestParams);
        ...
        fetchReports: async (tourId, overrideParams = {}, force = false) => {
          ...
          const url = `${URL_AFTER_API}/${tourId}/tour-insights/reports`;
        ...
        fetchFaqs: async (tourId, overrideParams = {}, force = false) => {
          ...
          // Use the new cache key function that includes search
          ...
          // Build request parameters including search
          const requestParams: Record<string, unknown> = {
            page: params.page,
            limit: params.limit,
          };
          ...
          const url = `${URL_AFTER_API}/${tourId}/tour-insights/faqs`;
          const reqKey = makeRequestKey("GET", url, requestParams);
          if (!force && inFlightRequests.has(reqKey))
            return inFlightRequests.get(reqKey) as Promise<
          ...
          const p = api
            .get<ApiResponse<GetTourFaqsResponse>>(url, { params: requestParams })
            .then((res) => {
              if (!(res.data && res.data.data)) {
                throw new Error("Invalid response body.")
              }
        ...
        fetchBookings: async (tourId, overrideParams = {}, force = false) => {
          const state = get();
          if (!state.params.tourBookings) state.params.tourBookings = {};
          if (!state.params.tourBookings[tourId])
            state.params.tourBookings[tourId] = { ...defaultPagination, search: "" };
          ...
          const url = `${URL_AFTER_API}/${tourId}/tour-insights/bookings`;
          const requestParams: Record<string, unknown> = {
            page: params.page,
            limit: params.limit,
          };
