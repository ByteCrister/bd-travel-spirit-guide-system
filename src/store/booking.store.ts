...
export const API_BOOKINGS_SUMMARY_URL = '/mock/operations/bookings/summary';
...
const generateCacheKey = (filters: BookingsFilterState, suffix = 'list'): string =>
...
export const useBookingStore = create<BookingsStoreState>()(
    devtools(
        (set, get) => ({
            ...
            resetFilters: () => {
                set({ filters: { ...DEFAULT_FILTERS } }, false, 'bookingStore/resetFilters');
            },
            ...
            invalidateBookingsCache: () => {
                set((state) => {
                    ...
                    return { cache: newCache };
                }, false, 'bookingStore/invalidateBookingsCache');
            },
            ...
            fetchBookings: async (ignoreCache = false) => {
                ...
                if (!ignoreCache) {
                    ...
                    if (cachedItem && isCacheValid(cachedItem) && cachedItem.type === 'bookings') {
                        set({
                            bookings: cachedItem.data.bookings,
                            pagination: cachedItem.data.pagination,
                            isLoading: false,
                            error: null,
                        }, false, 'bookingStore/fetchBookings/cache-hit');
                        return;
                    }
                }

                set({ isLoading: true, error: null }, false, 'bookingStore/fetchBookings/start');
                ...
                try {
                    ...
                    if (filters.paymentStatus) {
                        const pStatuses = Array.isArray(filters.paymentStatus) ? filters.paymentStatus : [filters.paymentStatus];
                    ...
                    if (!response.data.error && response.data.data) {
                        // ✅ Correct extraction: response.data.data contains { data: IBookingPopulated[], meta: PaginationMeta }
                        ...
                        }), false, 'bookingStore/fetchBookings/success');
                    ...
                    set({ isLoading: false, error: errorMessage }, false, 'bookingStore/fetchBookings/error');
            ...
            fetchSummary: async (ignoreCache = false) => {
                ...
                if (!ignoreCache) {
                    const cachedItem = cache.get(cacheKey);
                    if (cachedItem && isCacheValid(cachedItem) && cachedItem.type === 'summary') {
                        set({ summary: cachedItem.data, isSummaryLoading: false }, false, 'bookingStore/fetchSummary/cache-hit');
                        return;
                    }
                }

                set({ isSummaryLoading: true, error: null }, false, 'bookingStore/fetchSummary/start');

                try {
                    const response = await api.get<BookingsSummaryApiResponse>(API_BOOKINGS_SUMMARY_URL);

                    if (!response.data.error && response.data.data) {
                        // ✅ Correct extraction: response.data.data contains { data: BookingStatusCount }
                        ...
                        }), false, 'bookingStore/fetchSummary/success');
                    } else {
                        throw new Error(response.data.error || 'Failed to fetch summary');
                    }
                } catch (error) {
                    const errorMessage = extractErrorMessage(error);
                    set({ isSummaryLoading: false, error: errorMessage }, false, 'bookingStore/fetchSummary/error');
                    toast.error(errorMessage);
                }
            },
            ...
            // CANCEL BOOKING
            // ============================================
            cancelBooking: async (bookingId: string, payload: CancelBookingRequest) => {
                set({ isCancelling: true, error: null }, false, 'bookingStore/cancelBooking/start');
                try {
                    const url = `${API_BOOKINGS_URL}/${bookingId}/cancel`;
                    const response = await api.post<BookingActionApiResponse>(url, payload);
                    ...
                    if (!response.data.error) {
                        ...
                        set((state) => ({
                            ...
                            bookings: state.bookings.map((b) =>
                                b._id === bookingId
                                    ? { ...b, status: BOOKING_STATUS.CANCELLED as BookingStatus }
                                    : b
                            ),
                        }), false, 'bookingStore/cancelBooking/success');
                    ...
                    set({ isCancelling: false, error: msg }, false, 'bookingStore/cancelBooking/error');
                    toast.error(msg);
                }
            },
            ...
            // REFUND BOOKING
            // ============================================
            refundBooking: async (bookingId: string, payload: RefundBookingRequest) => {
                set({ isRefunding: true, error: null }, false, 'bookingStore/refundBooking/start');
                try {
                    ...
                    if (!response.data.error) {
                        ...
                        }), false, 'bookingStore/refundBooking/success');
                        get().invalidateBookingsCache();
                        get().fetchSummary(true);
                    ...
                    const msg = extractErrorMessage(error);
                    set({ isRefunding: false, error: msg }, false, 'bookingStore/refundBooking/error');
                    toast.error(msg);
                }
            },
...
export const useBookingsPagination = () => useBookingStore((state) => state.pagination);
export const useBookingsFilters = () => useBookingStore((state) => state.filters);
export const useBookingsLoading = () => useBookingStore((state) => state.isLoading);
export const useBookingsSummary = () => useBookingStore((state) => state.summary);
export const useBookingsError = () => useBookingStore((state) => state.error);
export const useIsCancelling = () => useBookingStore((state) => state.isCancelling);
export const useIsRefunding = () => useBookingStore((state) => state.isRefunding);
...
export const useBookingActions = () => {
    ...
    const invalidateBookingsCache = useBookingStore((state) => state.invalidateBookingsCache);
